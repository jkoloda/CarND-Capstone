...
class WaypointUpdater(object):
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            # If system already initialised...
            if self.pose and self.base_lane:
                # Get closest waypoint and publish its index
    ...
    def get_closest_waypoint_idx(self):
        """Get closest waypoint index *ahead* of car."""
        ...
        prev_vect = np.array(prev_coord)    # Previous waypoint
        pos_vect = np.array([x,y])          # Vehicle position

        # Is the closest waypoints behind or ahead of car position?
    ...
    def publish_waypoints(self, closest_idx):
        """Publish the slice of waypoints ahead of vehicle."""
        final_lane = self.generate_lane()
        self.final_waypoints_pub.publish(final_lane)


    def generate_lane(self):
        """Generate lane ahead of car."""
        lane = Lane()
        closest_idx = self.get_closest_waypoint_idx()
        farthest_idx = closest_idx + LOOKAHEAD_WPS
        base_waypoints = self.base_lane.waypoints[closest_idx:farthest_idx]

        # If traffic red light on we need to slow down (or to stop eventually)
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        """Slow down (or even stop) before red traffic light."""
        # Stop two waypoints back from line so front of car is at stop line
        # This index is relative to the base_lane slice
    ...
    def waypoints_cb(self, waypoints):
        """Store base waypoints as KDTree."""
        self.base_lane = waypoints
        # Base waypoints don't change so grabbing them once is enough
