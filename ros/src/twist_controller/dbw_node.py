...
class DBWNode(object):
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            # Get predicted throttle, brake, and steering after initialisation
            # rospy.loginfo('DBW Info:')
            ...
            if not None in (self.current_vel,
                            self.linear_vel,
                            self.angular_vel):
                output = self.controller.control(self.current_vel,
                                                 self.dbw_enabled,
                                                 self.linear_vel,
                                                 self.angular_vel)
                self.throttle, self.brake, self.steering = output                
